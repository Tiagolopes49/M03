...
def PedirOrdenado():
    """Esta função pede ao utilizado quanto ganha por cada hora de trabalh"""
...
def MostrarVencimento(nome,horas,ordenado_por_hora):
    ...
    if horas_extra_extra<0:
        horas_extra_extra=0
    ordenado=ordenado_por_hora*horas_normais+ordenado_por_hora*1.25*horas_extra+ordenado_por_hora*1.50*horas_extra_extra
    print(f"{nome} vai receber {ordenado}€ pelas {horas} de trabalho.")
